...
typedef int (*dinput8_GetDeviceState_typedef)(void *this, int size, void *to);
...
BOOL APIENTRY DllMain(HMODULE hModule, DWORD ul_reason_for_call, LPVOID lpReserved) {
	...
	switch(ul_reason_for_call) {
		...
		// Return FALSE to fail DLL load.
			...
			if (!ori_dsound_library || !ms_ori_lib || !ori_dinput8_library) {
				MessageBox(NULL, "Error: loading one of the dll files!", "Err", MB_OK | MB_ICONERROR);
				return FALSE;
			}

			ori_dinput8_GetDeviceState = (dinput8_GetDeviceState_typedef)GetProcAddress(ori_dinput8_library, "DirectInput8Create");
			ori_dinput8_GetDeviceState -= 0x62B1; //Get device state have 0x62B1 offset from the exported function DirectInput8Create
			...
			VirtualProtectEx(Curr_handle, (void *)0x00468135, 5, PAGE_EXECUTE_READWRITE, &OldProtect);
			//VirtualProtectEx(Curr_handle, (void *)0x005391CC, 4, PAGE_EXECUTE_READWRITE, &OldProtect);
			...
			*((unsigned int *)(to_patch_2+6)) = (unsigned int *)( (&music_handler) );
			...
			to_patch_3[0] = 0xba;
			*((unsigned int *)(to_patch_3+1)) = (unsigned int *)( (&DETECT_INPUT) );

			//*((unsigned int *)(to_patch_4)) = (unsigned int *)( (&srand_wrapper) );
